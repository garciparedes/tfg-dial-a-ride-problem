  ...
  \label{chap:formulation}
    ...
    \label{sec:formulation_contextualization}
      ...
      \label{sec:formulation_combinatorial_problems}
        ...
        \paragraph{}
        Desde el punto de vista de su complejidad algorítmica, la gran mayoría de \emph{problemas de optimización combinatoria} pertenecen a la clase de \emph{problemas no determinísticamente polinómicos (o NP)}. Esta categoría se caracterizar por generar funciones de naturaleza no polinómica cuando se estudia el número de operaciones necesarias para que estos sean resueltos, tomando como soporte el tamaño de los datos de entrada. Estas funciones tienen comportamientos de tipo exponencial en muchos casos. Este tipo de comportamientos se traducen en una cantidad de recursos computacionales muchas veces inabarcables cuando el tamaño de la entrada es relativamente elevado. A modo de ejemplo supóngase un caso extremo, supóngase un algoritmo que tome como entrada un vector de valores numéricos, cuyo coste computacional sigue una distribución $f(x) = 2 ^ n$. Para vectores de tamaño $n = 3$, se requerirá unas $f(3) = 8$ operaciones para ser ejecutado. Sin embargo, para vectores de tamaño $n = 100$ el número de operaciones asciende a $f(100) \approx 1.2676506*10^{30}$. Tal y como se puede intuir, en la actualidad no existe ninguna máquina capaz de resolver tal número de operaciones en un tiempo razonable.

        \paragraph{}
        Tal y como se ha remarcado a lo largo de todo el apartado, es especialmente importante indicar que la mayor dificultad surgida al añadir las restriciones en el soporte de las variables de decisión genera un mayor espacio de puntos de evaluación, lo cual complica extremadamente los métodos de generación de soluciones. Por contra, esta dificultad amplia en gran medida la capacidad de representación de los modelos, lo cual es algo beneficioso ya que permite modelizar situaciones reales que de otra manera no se podrían enmarcar sobre el contexto de problemas de optimización con estructura lineal.
      ...
      \label{sec:formulation_dial_a_ride_problems}
        ...
        \paragraph{}
        Tras haber llevado a cabo una breve introducción al problema \emph{problema Dial-a-Ride}, lo siguiente es profundizar en la modelación del mismo como un problema de \emph{optimización lineal mixto}, para lo cual se dedica por completo el \cref{sec:formulation_basic_formulation}. Sin embargo, antes de ello es necesario hacer una aclaración acerca del significado de las palabras clave más relevantes, así como la nomenclatura que se tratará de seguir en el resto del trabajo. Para ello se destina el próximo apartado (\emph{Notación}).
    ...
    \label{sec:formulation_basic_formulation}
      ...
      \paragraph{}
      A lo largo del apartado se han indicado las restricciones que componen la modelización básica del problema \emph{Dial-a-Ride} sobre el marco de \emph{programación lineal mixta y entera (MILP)}. Una vez hecho esto, tan solo queda remarcar los aspectos más destacados del capítulo en el apartado de conclusiones.

    \section{Conclusiones}
    \label{sec:formulation_conclusions}

      \paragraph{}
      Durante el capítulo se han comentado distintos aspectos necesarios para la compresnsión del \emph{problema Dial-a-Ride}, desde su contextualización siguiendo una perspectiva descendente. Seguidamente se ha continuado describiendo las palabras clave así como las estructuras matemáticas que se usarían posteriormente durante el apartado dedicado a la formulación básica del problema. En este, además se ha llevado un análisis acerca de las restriciones que componen la modelización como \emph{problema lineal mixto} agrupadas por naturaleza. De esta manera, se ha tratado de exponer todas aquellas ideas que después son de gran relevancia desde una perspectiva de resolución.

      \paragraph{}
      Algo a remarcar es que el \emph{problema Dial-a-Ride} puede ser extendido de tal manera que se incluyan restricciones que representen otras situaciones más complejas, tales como la de imponer restricciones de autonomía a los vehículos siguiendo técnicas como las descritas en \cite{erdougan2012green} o imponer relaciones de orden entre los viajes.

      \paragraph{}
      A pesar de que a lo largo del capítulo la problemática se haya definido siempre desde una perspectiva estática (donde se suponen conocidos todos los viajes y vehículos disponibles \emph{a-priori}), también es de gran interés el estudio de la problemática desde una perspectiva dinámica (donde los viajes no son conocidos al principio de la ejecución, sino que estos van apareciendo de manera estocástica sin conocimiento previo), para lo cual se recomienda consultar \cite{santos2015taxi}.
